...
public partial class OverlayWindow : Window
{
    ...
    internal void Present(string display, string app, DisplayRule rule)
    {
        ...
        Source.Text = rule.Level >= 3 ? $"{ShortcutSource(display)} · {app}" : rule.Level == 2 ? app : string.Empty; Action.Text = rule.Level >= 3 ? (string.IsNullOrWhiteSpace(rule.Description) ? "快捷键" : rule.Description) : string.Empty; Height = compact ? 38 : rule.Level >= 3 ? 96 : 70; Width = compact ? 1 : 260; SizeToContent = compact ? SizeToContent.Width : SizeToContent.Manual; if (compact) { Show(); UpdateLayout(); Position(); } else { Position(); Show(); } _hide.Stop(); _hide.Start();
    }
    private void Position() { var screens = Forms.Screen.AllScreens; var screen = screens[Math.Clamp(App.Settings.MonitorIndex, 0, screens.Length - 1)]; var area = screen.WorkingArea; var gap = 18; Left = App.Settings.OverlayPosition switch { "左上" => area.Left + gap, "右上" => area.Right - Width - gap, "左下" => area.Left + gap, _ => area.Right - Width - gap }; Top = App.Settings.OverlayPosition switch { "左上" or "右上" => area.Top + gap, _ => area.Bottom - Height - gap }; }

...
internal sealed class KeyboardHook : IDisposable
{
    ...
    private nint Callback(int code, nint wParam, nint lParam) { if (code >= 0 && (wParam == WmKeyDown || wParam == WmSysKeyDown)) { var data = Marshal.PtrToStructure<Kbd>(lParam); var key = KeyInterop.KeyFromVirtualKey((int)data.VkCode); var mods = Keyboard.Modifiers; var info = ForegroundInfo(); var rule = App.ResolveRule(info.Path); var text = Format(key, mods, rule.ShowSingleKeys, rule.Level >= 3); var catalog = !IsModifierKey(key) && mods != ModifierKeys.None && !IsTextInputCombination(key, mods) ? Format(key, mods, true, false) : string.Empty; if (!string.IsNullOrWhiteSpace(text)) KeyPressed?.Invoke(this, new KeyPressedEventArgs(text, info.Name, info.Path, catalog)); } return CallNextHookEx(_hook, code, wParam, lParam); }
